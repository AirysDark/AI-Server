...
public partial class MainWindow : Window
{
    ...
    private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        {
            LoginStatus.Text = "Server unavailable: " + ex.Message;
        }
    }

    private async void Login_Click(object sender, RoutedEventArgs e)
    {
        await Authenticate(async () => await _api.LoginAsync(EmailBox.Text.Trim(), PasswordBox.Password));
    }

    private async void Register_Click(object sender, RoutedEventArgs e)
    {
        await Authenticate(async () => await _api.RegisterAsync(EmailBox.Text.Trim(), PasswordBox.Password, UsernameBox.Text.Trim()));
    }
    ...
    private async Task LoadConversationAsync()
    {
        ...
        foreach (var entry in history.Conversation ?? [])
        {
            if (!string.IsNullOrWhiteSpace(entry.User))
                Messages.Add(new ChatMessage { Role = "You", Text = entry.User });
            if (!string.IsNullOrWhiteSpace(entry.Ai))
                Messages.Add(new ChatMessage { Role = AiNameText.Text, Text = entry.Ai, Image = entry.Image });
        }
    ...
    private async void AiSelector_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
    {
        ...
        {
            _changingAi = true;
            await _api.SelectAiAsync(ai.AiId);
            await LoadCurrentAiAsync();
        }
        catch (Exception ex)
        {
    ...
    {
        SettingsColumn.Width = SettingsColumn.Width.Value == 0 ? new GridLength(330) : new GridLength(0);
    }
    ...
    private void Attach_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            _attachedImage = dialog.FileName;
            ServerStatus.Text = "Attached: " + Path.GetFileName(_attachedImage);
        }
    }
    ...
    private async void Send_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var reply = string.IsNullOrWhiteSpace(response.Reply) ? "No response returned." : response.Reply;
            Messages.Add(new ChatMessage { Role = AiNameText.Text, Text = reply, Image = response.Image });
            MessagesList.ScrollIntoView(Messages[^1]);
    ...
    private async Task SetAiPhotoAsync(string? path)
    {
        ...
        try
        {
            var bytes = await new HttpClient().GetByteArrayAsync(_api.ResolveUrl(path));
