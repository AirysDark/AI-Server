...
public sealed class ApiClient(string? baseUrl = null)
{
    ...
    public async Task SelectAiAsync(string aiId, CancellationToken ct = default)
    {
        ...
        if (result?.Ok != true)
            throw new ApiException("AI could not be selected", HttpStatusCode.BadRequest);
        ActiveAiId = aiId;
    }
    ...
    public async Task<string> CreateAiAsync(CancellationToken ct = default)
    {
        ...
        if (result?.Ok != true || string.IsNullOrWhiteSpace(result.AiId))
            throw new ApiException("AI could not be created", HttpStatusCode.BadRequest);
        ActiveAiId = result.AiId;
        return result.AiId;
    }
    ...
    public async Task DeleteAiAsync(string aiId, CancellationToken ct = default)
    {
        ...
        if (result?.Ok != true)
            throw new ApiException("AI could not be deleted", HttpStatusCode.BadRequest);
    }

    public async Task<AiSettings> GetSettingsAsync(CancellationToken ct = default) =>
        await GetAsync<AiSettings>("api/settings", ct) ?? new AiSettings();

    public async Task<AiSettings> SaveSettingsAsync(AiSettings settings, CancellationToken ct = default) =>
        await PostAsync<AiSettings, AiSettings>("api/settings", settings, ct) ?? settings;

    public async Task<ChatResponse> ChatAsync(string message, string? imagePath = null, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(imagePath))
            return await PostAsync<ChatResponse, object>("chat", new { message }, ct) ?? new ChatResponse();
        ...
        if (!response.IsSuccessStatusCode)
            throw new ApiException(result?.Error ?? response.ReasonPhrase ?? "Chat request failed", response.StatusCode);
    ...
    private static string GetError<T>(T? result, HttpResponseMessage response) =>
        result is ErrorResponse error && !string.IsNullOrWhiteSpace(error.Error)
            ? error.Error
            : response.ReasonPhrase ?? "Server request failed";
