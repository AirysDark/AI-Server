"""Conversation bridge: persist directly to the selected archive file."""
import json
import threading
import time
import os
...
from core.ai_manager import conversations_root, load_archived_conversation, save_archived_conversation
...
def save_message(uid, ai_id, user_message, ai_reply, image=None):
    """Append the complete exchange directly to the selected C-*.json."""
    cid = selected_id()
    if not cid:
        raise IOError("No conversation selected")
    data = load_archived_conversation(uid, ai_id, cid)
    if data is None:
        data = {"conversation": [], "memory": {}, "proactive_state": {}, "created": time.time()}
    now = time.time()
    user_text, ai_text = str(user_message or ""), str(ai_reply or "")
    ...
    if not saved or not saved.get("conversation") or saved["conversation"][-1].get("user_message") != user_text or saved["conversation"][-1].get("ai_reply") != ai_text:
...
def _install_context_wrappers():
    ...
    def do_get(handler):
        ...
        if path == "/api/user":
            ...
            data = load_archived_conversation(uid, ai_id, cid) if cid else None
            if data is None: data = {"conversation":[],"memory":{},"proactive_state":{}}
            return handler.send_json(data, uid, 200, ai_id)
    ...
    def do_post(handler):
        ...
        if path == "/chat":
            ...
            if not _context.conversation_id:
                _context.conversation_id = None
            try:
    ...
    server_impl._direct_chat_archive_installed = True
