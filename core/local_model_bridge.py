...
def _store_model(uid, ai_id, name, reader, content_length):
    if content_length <= 0: raise ValueError("No model file uploaded")
    if content_length > MAX_LOCAL_MODEL_BYTES: raise ValueError("Local model is larger than the 2 GB upload limit")
    ...
    try:
        with open(temp_path, "wb") as output:
            while received < content_length:
                ...
                if not chunk: raise IOError("Upload ended before the complete model was received")
                output.write(chunk); received += len(chunk)
        if received != content_length: raise IOError("Incomplete model upload")
...
def _server_models():
    ...
    for name in sorted(os.listdir(MODELS_DIR), key=str.lower):
        ...
        if os.path.isfile(path):
            try: result.append({"filename": name, "size": os.path.getsize(path)})
            except OSError: pass
    return result
...
def _select_server_model(uid, ai_id, filename):
    ...
    if not filename.lower().endswith(".gguf") or not os.path.isfile(path) or os.path.dirname(path) != os.path.abspath(MODELS_DIR):
...
def handle_wsgi_upload(environ, start_response):
    ...
    handler = CookieHandler(environ.get("HTTP_COOKIE", "")); uid, ai_id = active_ai(handler)
    if not uid:
...
def _install(handler_class):
    if getattr(handler_class, "_local_model_routes_installed", False): return
    original_post = handler_class.do_POST
    original_get = handler_class.do_GET

    def do_get(handler):
        path = handler.path.split("?", 1)[0]
        if path not in ("/api/local-model", "/api/local-models"): return original_get(handler)
        ...
        if path == "/api/local-models":
            return handler.send_json({"ok": True, "models": _server_models(), "directory": MODELS_DIR}, uid, 200, ai_id)
        return handler.send_json(_status(uid, ai_id), uid, 200, ai_id)

    def do_post(handler):
        path = handler.path.split("?", 1)[0]
        if path not in ("/api/local-model", "/api/local-models"): return original_post(handler)
        uid, ai_id = active_ai(handler)
        if not uid: return handler.send_json({"error": "Authentication required"}, status=401)
        try:
            if path == "/api/local-models":
                ...
                return handler.send_json(_select_server_model(uid, ai_id, data.get("filename")), uid, 200, ai_id)
