...
TEXT_MODELS = [
    os.getenv("AI_AI_MODEL", "Qwen/Qwen2.5-7B-Instruct"), "Qwen/Qwen3-8B", "Qwen/Qwen3-4B-Instruct-2507", "Qwen/Qwen2.5-7B-Instruct",
    "Qwen/Qwen2.5-3B-Instruct", "Qwen/Qwen2.5-1.5B-Instruct", "meta-llama/Llama-3.3-70B-Instruct", "meta-llama/Llama-3.1-8B-Instruct",
    "meta-llama/Llama-3.2-3B-Instruct", "microsoft/Phi-3.5-mini-instruct", "google/gemma-3-4b-it", "google/gemma-2-9b-it",
    "mistralai/Mistral-7B-Instruct-v0.3", "HuggingFaceH4/zephyr-7b-beta", "openai/gpt-oss-20b", "openai/gpt-oss-120b",
    "deepseek-ai/DeepSeek-R1-Distill-Qwen-7B", "deepseek-ai/DeepSeek-R1-Distill-Qwen-14B", "moonshotai/Kimi-K2-Instruct", "zai-org/GLM-4.5-Air",
]
VISION_MODELS = [
    os.getenv("AI_AI_VISION_MODEL", "Qwen/Qwen2.5-VL-7B-Instruct"), "Qwen/Qwen2.5-VL-7B-Instruct", "Qwen/Qwen2.5-VL-3B-Instruct",
    "Qwen/Qwen2-VL-7B-Instruct", "meta-llama/Llama-3.2-11B-Vision-Instruct", "meta-llama/Llama-3.2-90B-Vision-Instruct",
    "google/gemma-3-12b-it", "google/gemma-3-4b-it", "mistralai/Pixtral-12B-2409",
]
_HEALTH = {}; _FAILURE_COOLDOWN = 300
...
def _is_vision_model(model):return any(x in model.lower() for x in ("vl","vision","pixtral","gemma-3"))
...
def _process_pending_feedback(settings):
    ...
    try:
        with open(os.path.join(root,"settings.json"),"w",encoding="utf-8") as f:
            import json;json.dump(settings,f,indent=4,ensure_ascii=False)
...
def ask_online(prompt,settings=None,knowledge="",image_path=None):
    ...
    token=str(settings.get("hf_token","")).strip() or os.getenv("HF_TOKEN")
    if not token:print("ONLINE AI: no Hugging Face token configured");return None
    description=settings.get("description","You are AI, a personal AI assistant.");personality=settings.get("personality","Helpful and friendly.");instructions=settings.get("instructions","");user_information=settings.get("user_information","");background=settings.get("background","")
    ai_gender=str(settings.get("ai_gender","")).strip().lower();user_gender=str(settings.get("user_gender","")).strip().lower();user_name=settings.get("user_name","");gender_guidance=""
    ...
    base64_img=encode_image(image_path);models=_unique((VISION_MODELS+_discover_models(token,True)) if base64_img else (TEXT_MODELS+_discover_models(token,False)))
    ...
    for model in models:
        ...
        try:
            ...
            if response.status_code<400 and data.get("choices"):
                _mark_success(model);reply=data["choices"][0]["message"]["content"];learn_online_response(prompt,reply,settings);print("ONLINE AI USING MODEL:",model);return reply
            _mark_failure(model,data);print("ONLINE AI MODEL FAILED:",model,data)
        except requests.RequestException as e:_mark_failure(model,e);print("ONLINE AI ERROR:",model,e)
        except Exception as e:_mark_failure(model,e);print("ONLINE AI ERROR:",model,e)
    return None
