...
DEFAULT_IMAGE_MODEL = os.getenv("AI_IMAGE_MODEL", "black-forest-labs/FLUX.1-schnell")
...
def generate_image_bytes(token, prompt, model=None, timeout=120):
    ...
    model = str(model or DEFAULT_IMAGE_MODEL).strip() or DEFAULT_IMAGE_MODEL
    ...
    if response.status_code >= 400:
        ...
        raise RuntimeError("Hugging Face image generation failed: HTTP %s %s" % (response.status_code, str(detail)[:500]))
