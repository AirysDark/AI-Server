...
The AI root current.json is the live working conversation. The
conversations/ directory contains saved chat snapshots. Switching chats
archives the live current.json, then copies the selected snapshot into the
live current.json file.
"""
...
def _paths(uid,ai_id): return ai_root(uid,ai_id), conversations_root(uid,ai_id)
def _title(data):
...
def _archive_current(uid,ai_id):
    _,archive=_paths(uid,ai_id); data=load_conversation(uid,ai_id)
    if not data.get("conversation"): return None
    os.makedirs(archive,exist_ok=True); cid="C-"+uuid.uuid4().hex[:16]; now=time.time()
    record={"conversation_id":cid,"title":_title(data),"created":data.get("created",now),"updated":data.get("updated",now),"data":data}
    save_json(os.path.join(archive,cid+".json"),record); return record

def list_chats(uid,ai_id):
    _,archive=_paths(uid,ai_id); os.makedirs(archive,exist_ok=True); result=[]
...
def new_chat(uid,ai_id):
    _archive_current(uid,ai_id); now=time.time(); save_conversation_data(uid,ai_id,{"conversation":[],"memory":{},"proactive_state":{},"created":now,"updated":now}); return {"ok":True,"conversation_id":"current"}

def open_chat(uid,ai_id,conversation_id):
    if conversation_id=="current": return load_conversation(uid,ai_id)
    data=load_archived_conversation(uid,ai_id,conversation_id)
    if data is None: return None
    current=load_conversation(uid,ai_id)
    if current.get("conversation"): _archive_current(uid,ai_id)
    data=dict(data); data["updated"]=time.time(); save_conversation_data(uid,ai_id,data); return data
...
def rename_chat(uid,ai_id,conversation_id,title):
    ...
    if conversation_id=="current":
        ...
        if not data.get("conversation"): return False
        data["title"]=title; data["updated"]=time.time(); save_conversation_data(uid,ai_id,data); return True
    _,archive=_paths(uid,ai_id); path=os.path.join(archive,_safe(conversation_id)+".json"); record=load_json(path,None)
    if not isinstance(record,dict) or not isinstance(record.get("data"),dict): return False
    record["title"]=title; record["updated"]=time.time(); record["data"]["title"]=title; record["data"]["updated"]=record["updated"]; save_json(path,record); return True
...
def install_handler_routes(handler_class,server_module):
    ...
    def do_post(self):
        ...
        if not conversation_id: return self.send_json({"ok":False,"error":"conversation_id required"},uid,400,ai_id)
        if path=="/api/chats/rename":
            if not rename_chat(uid,ai_id,conversation_id,data.get("title")): return self.send_json({"ok":False,"error":"Unable to rename conversation"},uid,400,ai_id)
