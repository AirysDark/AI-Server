...
bool Model::loadPmx(const std::string& path)
{
    ...
    if (!r.text(name, encoding) || !r.text(english, encoding) || !r.text(comment, encoding) || !r.text(englishComment, encoding)) return false;

    uint32_t vertexCount{};
    if (!r.u32(vertexCount) || vertexCount > 10000000) return false;
    _vertices.resize(vertexCount);
    for (auto& v : _vertices) {
        if (!r.vec3(v.position.x, v.position.y, v.position.z) || !r.vec3(v.normal.x, v.normal.y, v.normal.z) || !r.f32(v.uv.x) || !r.f32(v.uv.y)) return false;
        float f{};
    ...
    for (auto& value : _indices) { int32_t idx{}; if (!r.index(idx, vertexIndexSize, false) || idx < 0) return false; value = static_cast<uint32_t>(idx); }
    ...
    for (uint32_t i=0;i<materialCount;++i) {
        ...
        uint8_t sphereMode{}; if(!r.u8(sphereMode)) return false;
        uint8_t toonFlag{}; if(!r.u8(toonFlag)) return false;
        if(toonFlag==0) { if(!r.index(idx,textureIndexSize,true)) return false; }
        else { uint8_t toonTexture{}; if(!r.u8(toonTexture)) return false; }
    ...
    uint32_t morphCount{}; if(!r.u32(morphCount)||morphCount>1000000) return false;
    for(uint32_t i=0;i<morphCount;++i) {
        Morph m{}; if(!r.text(m.name,encoding)||!r.text(m.englishName,encoding)) return false;
        uint8_t panel{}, type{}; if(!r.u8(panel)||!r.u8(type)||!r.u32(m.offsetCount)) return false; m.panel=panel; m.type=type;
        for(uint32_t j=0;j<m.offsetCount;++j) {
            float f{}; int32_t idx{};
            if(type==0) { if(!r.index(idx,morphIndexSize,true)||!r.f32(m.weight)) return false; }
            else if(type==1) { if(!r.index(idx,vertexIndexSize,true)) return false; for(int n=0;n<3;++n)if(!r.f32(f))return false; }
            else if(type==2) { if(!r.index(idx,boneIndexSize,true)) return false; for(int n=0;n<7;++n)if(!r.f32(f))return false; }
            else if(type>=3&&type<=7) { if(!r.index(idx,vertexIndexSize,true))return false; for(int n=0;n<4;++n)if(!r.f32(f))return false; }
            else if(type==8) { if(!r.index(idx,materialIndexSize,true))return false; for(int n=0;n<16;++n)if(!r.f32(f))return false; if(!r.index(idx,textureIndexSize,true)||!r.index(idx,textureIndexSize,true))return false; uint8_t toon{};if(!r.u8(toon))return false;if(toon==0){if(!r.index(idx,textureIndexSize,true))return false;}else{uint8_t one{};if(!r.u8(one))return false;} }
            else return false;
        }
        _morphs.push_back(std::move(m));
    }

    uint32_t boneCount{}; if(!r.u32(boneCount)||boneCount>1000000)return false; _bones.reserve(boneCount);
    for(uint32_t i=0;i<boneCount;++i) {
        Bone b{};
        if(!r.text(b.name,encoding)||!r.text(b.englishName,encoding)||!r.vec3(b.position.x,b.position.y,b.position.z)||!r.index(b.parent,boneIndexSize,true))return false;
        uint16_t flags{};if(!r.u16(flags))return false;int32_t idx{};float f{};
        if(flags&0x0001){if(!r.index(idx,boneIndexSize,true))return false;}else{for(int n=0;n<3;++n)if(!r.f32(f))return false;}
        if(flags&(0x0100|0x0200)){if(!r.index(idx,boneIndexSize,true)||!r.f32(f))return false;}
        ...
        // PMX IK block: target bone, loop count, loop angle, then IK links.
        // Each link has a bone index and a byte indicating whether lower/upper
        // rotation limits follow (three floats each).
        if(flags&0x0020) {
            if(!r.index(idx,boneIndexSize,true))return false;
            uint32_t loopCount{};if(!r.u32(loopCount)||loopCount>100000)return false;
            if(!r.f32(f))return false;
            uint32_t linkCount{};if(!r.u32(linkCount)||linkCount>100000)return false;
            for(uint32_t k=0;k<linkCount;++k) {
                if(!r.index(idx,boneIndexSize,true))return false;
                uint8_t hasLimit{};if(!r.u8(hasLimit))return false;
                if(hasLimit)for(int n=0;n<6;++n)if(!r.f32(f))return false;
            }
        }
