...
{
    auto dc = static_cast<HDC>(_backBufferDC);
    auto bitmap = static_cast<HBITMAP>(_backBufferBitmap);
    auto oldBitmap = static_cast<HBITMAP>(_backBufferOldBitmap);
...
bool Renderer::ensureBackBuffer(int width, int height)
{
    ...
    auto oldBitmap = static_cast<HBITMAP>(SelectObject(memoryDC, bitmap));
...
{
    struct Projected { float x{}, y{}, depth{}, invDepth{}, u{}, v{}, nx{}, ny{}, nz{}; } p[3];
    ...
    auto project = [&](const Vertex& v) {
        ...
        q.depth = std::max(0.001f, cameraZ - v.position.z);
    ...
    for (int y=minY; y<=maxY; ++y) for (int x=minX; x<=maxX; ++x) {
        ...
        if (textured) {
            ...
            const uint32_t tx=std::min(texture->width-1,uint32_t(std::max(0.0f,uu*texture->width)));
            const uint32_t ty=std::min(texture->height-1,uint32_t(std::max(0.0f,vv*texture->height)));
            const uint8_t* s=&texture->pixels[(size_t(ty)*texture->width+tx)*4];
            sr=s[0]; sg=s[1]; sb=s[2];
            textureAlpha=texture->useAlpha?s[3]:255;
        }
        const uint8_t alpha=uint8_t((uint16_t(materialAlpha)*textureAlpha+127)/255);
        ...
        if (alpha < 255) {
            const uint32_t d=dst[pos], ia=255-alpha;
            dst[pos]=0xFF000000u|(((unsigned(r)*alpha+((d>>16)&255u)*ia)/255)<<16)|(((unsigned(g)*alpha+((d>>8)&255u)*ia)/255)<<8)|((unsigned(b)*alpha+(d&255u)*ia)/255);
            // Transparent MMD surfaces must not occlude geometry behind them.
            if (alpha >= 250) _zBuffer[pos]=depth;
        } else {
            dst[pos]=0xFF000000u|(uint32_t(r)<<16)|(uint32_t(g)<<8)|uint32_t(b);
            _zBuffer[pos]=depth;
        }
    }
}
...
void Renderer::loadModelTextures(const Model& model)
{
    ...
    for (size_t i=0;i<model.textures().size();++i) {
        ...
        if (!std::filesystem::exists(path)) {
            auto fallback=std::filesystem::path(_textureRoot)/rel.filename();
...
void Renderer::draw(const Model& model)
{
    ...
    // Morphs are owned by Model and update the live vertex/material arrays.
    // Wire the renderer to consume that live state every frame.
    const_cast<Model&>(model).updateMorphs();
    ...
    // PMX indices are stored in the opposite winding expected by this rasterizer.
    ...
    for (const auto& mat:mats) {
        ...
        for (size_t j=0;j+2<count;j+=3) {
            ...
            if (ia<verts.size()&&ib<verts.size()&&ic<verts.size())
                drawTexturedTriangle(verts[ia],verts[ib],verts[ic],mat,tex);
        }
        offset+=count;
    }
    ...
    if (dc) {
        ...
        RECT tr{8,8,width-8,32};
        DrawTextW(dc,L"Gene Runtime - PMX",-1,&tr,DT_LEFT|DT_NOPREFIX);
    }
    present();
}
...
void Renderer::paint()
{
    ...
    if (dc&&w>0&&h>0&&ensureBackBuffer(w,h)) BitBlt(dc,0,0,w,h,static_cast<HDC>(_backBufferDC),0,0,w,h,SRCCOPY);
    EndPaint(hwnd,&ps);
}
...
void Renderer::setWindowTitle(const std::string& title)
{
    HWND hwnd=static_cast<HWND>(_window);
    if (!hwnd) return;
    const int required=MultiByteToWideChar(CP_UTF8,0,title.data(),int(title.size()),nullptr,0);
    if (required<=0) { SetWindowTextW(hwnd,L"Gene Runtime"); return; }
    std::wstring wide(size_t(required),L'\0');
    MultiByteToWideChar(CP_UTF8,0,title.data(),int(title.size()),wide.data(),required);
...
{
    HWND hwnd=static_cast<HWND>(_window);
    _window=nullptr; _running=false;
    destroyBackBuffer();
