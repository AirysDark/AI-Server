...
void Renderer::draw(const Model& model)
{
    ...
    if (!bones.empty()) {
        for (auto& v:skinned) {
            float sum=0.0f;
            for (int n=0;n<4;++n) {
                if (v.boneIndices[n] >= 0 && size_t(v.boneIndices[n]) < bones.size()) {
                    v.boneWeights[n]=std::max(0.0f,v.boneWeights[n]);
                    sum+=v.boneWeights[n];
                } else {
                    v.boneIndices[n]=-1;
                    v.boneWeights[n]=0.0f;
                }
            }
            if (sum>0.000001f) {
                const float inv=1.0f/sum;
                for (float& w:v.boneWeights) w*=inv;
            }
        }
    }
    ...
    for (const auto& mat:mats) {
        ...
        for (size_t j=0;j+2<count;j+=3) {
            // Babylon-MMD reverses the PMX index winding once while building geometry.
