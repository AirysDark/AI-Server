...
void Renderer::destroyBackBuffer()
{
    ...
    _backBufferDC = nullptr; _backBufferBitmap = nullptr; _backBufferOldBitmap = nullptr;
    _backBufferPixels = nullptr; _bufferWidth = 0; _bufferHeight = 0; _zBuffer.clear();
}
...
bool Renderer::ensureBackBuffer(int width, int height)
{
    ...
    _backBufferDC = memoryDC; _backBufferBitmap = bitmap; _backBufferOldBitmap = oldBitmap;
    _backBufferPixels = pixels; _bufferWidth = uint32_t(width); _bufferHeight = uint32_t(height);
...
bool Renderer::initialize(uint32_t width, uint32_t height)
{
    ...
    WNDCLASSW wc{}; wc.lpfnWndProc = GeneWndProc; wc.hInstance = instance;
    wc.lpszClassName = className; wc.hCursor = LoadCursorW(nullptr, IDC_ARROW); wc.hbrBackground = nullptr;
    RegisterClassW(&wc);
...
void Renderer::clearBackBuffer(uint32_t color)
{
    ...
    std::fill(dst, dst + size_t(_bufferWidth) * size_t(_bufferHeight), color);
                                    ...
                                    const Material& material, const TextureInfo* texture)
{
    ...
    auto* dst = static_cast<uint32_t*>(_backBufferPixels);
    const bool textured = texture && texture->loaded();
...
{
    if (!_window) return;
    HWND hwnd=static_cast<HWND>(_window); RECT client{}; GetClientRect(hwnd,&client);
    ...
    clearBackBuffer(0xFFF2F2F2u); loadModelTextures(model);
    const auto& verts=model.vertices(); const auto& idx=model.indices();
    ...
    _modelCenterX=(minX+maxX)*0.5f; _modelCenterY=(minY+maxY)*0.5f; _modelCenterZ=(minZ+maxZ)*0.5f;
    const float span=std::max({maxX-minX,maxY-minY,maxZ-minZ,0.001f});
    _cameraDistance=span*2.0f; _focalLength=float(std::min(_bufferWidth,_bufferHeight))*0.92f;
    const auto& mats=model.materials(); size_t offset=0;
    for (const auto& mat:mats) {
        if (offset>=idx.size()) break;
        const size_t count=std::min<size_t>(mat.indexCount,idx.size()-offset); const TextureInfo* tex=nullptr;
        ...
        for (size_t j=0;j+2<count;j+=3) { const uint32_t ia=idx[offset+j],ib=idx[offset+j+1],ic=idx[offset+j+2]; if (ia<verts.size()&&ib<verts.size()&&ic<verts.size()) drawTexturedTriangle(verts[ia],verts[ib],verts[ic],mat,tex); }
        offset+=count;
    }
    HDC dc=static_cast<HDC>(_backBufferDC); if (dc) { SetBkMode(dc,TRANSPARENT); SetTextColor(dc,RGB(30,30,30)); RECT tr{8,8,width-8,32}; DrawTextW(dc,L"Gene Runtime - PMX",-1,&tr,DT_LEFT|DT_NOPREFIX); }
    present();
}

void Renderer::paint()
{
    HWND hwnd=static_cast<HWND>(_window); if (!hwnd) return;
    PAINTSTRUCT ps{}; HDC dc=BeginPaint(hwnd,&ps); RECT r{}; GetClientRect(hwnd,&r);
    const int w=r.right-r.left,h=r.bottom-r.top;
    if (dc&&w>0&&h>0&&ensureBackBuffer(w,h)) BitBlt(dc,0,0,w,h,static_cast<HDC>(_backBufferDC),0,0,SRCCOPY);
    EndPaint(hwnd,&ps);
}
...
{
    if (!_window) return; const int n=MultiByteToWideChar(CP_UTF8,0,title.c_str(),-1,nullptr,0); if (n<=0) return;
    std::wstring w(size_t(n),L'\0'); MultiByteToWideChar(CP_UTF8,0,title.c_str(),-1,w.data(),n); SetWindowTextW(static_cast<HWND>(_window),w.c_str());
}

void Renderer::shutdown()
{
    HWND hwnd=static_cast<HWND>(_window); _window=nullptr; _running=false; destroyBackBuffer(); if (hwnd) DestroyWindow(hwnd);
}
}
