...
def _auth_route(environ, start_response, method, path, body):
    if path not in ("/api/auth/register", "/api/auth/forgot-password", "/api/auth/reset-password"):
        return None
    ...
    if data is None:
        return _direct_json(environ, start_response, "400 Bad Request", {"ok": False, "error": "Invalid JSON"})
    ...
    if path == "/api/auth/register":
        ...
        if not valid_email(email) or len(password) < 8:
            return _direct_json(environ, start_response, "400 Bad Request", {"error": "Valid email and password of at least 8 characters required"})
        if password != confirm:
            return _direct_json(environ, start_response, "400 Bad Request", {"error": "Passwords do not match"})
        ...
        if existing:
            return _direct_json(environ, start_response, "409 Conflict", {"error": "Account already exists"})
        ...
        headers = [
            ("Set-Cookie", f"AI_session={session}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000"),
            ("Set-Cookie", f"AI_user={uid}; Path=/; SameSite=Lax; Max-Age=31536000"),
        ]
    ...
    if path == "/api/auth/forgot-password":
        ...
        if not valid_email(email):
            return _direct_json(environ, start_response, "400 Bad Request", {"ok": False, "error": "Valid email address required"})
        try:
            request_reset(email)
        except Exception as exc:
            print("PASSWORD RESET EMAIL ERROR:", exc)
            return _direct_json(environ, start_response, "500 Internal Server Error", {"ok": False, "error": "Unable to send reset email right now"})
    ...
    confirm = str(data.get("confirm_password", ""))
    if password != confirm:
        return _direct_json(environ, start_response, "400 Bad Request", {"ok": False, "error": "Passwords do not match"})
    ok, message = reset_password(token, password)
    if not ok:
        return _direct_json(environ, start_response, "400 Bad Request", {"ok": False, "error": message})
...
def _conversation_route(environ, start_response, method, path, body):
    if path not in ("/api/chats", "/api/chats/new", "/api/chats/open", "/api/chats/rename"):
        return None
    ...
    request = CookieHandler(environ.get("HTTP_COOKIE", ""))
    uid = current_user(request)
    if not uid:
        return _direct_json(environ, start_response, "401 Unauthorized", {"ok": False, "error": "Authentication required"})

    _, ai_id = active_ai(request)
    if not ai_id:
        ai_id = ensure_first_ai(uid)

    if method == "GET" and path == "/api/chats":
        return _direct_json(environ, start_response, "200 OK", {"ok": True, "chats": chats_api.list_chats(uid, ai_id), "ai_id": ai_id})

    data = _read_json(body)
    if data is None:
        return _direct_json(environ, start_response, "400 Bad Request", {"ok": False, "error": "Invalid JSON"})

    if method == "POST" and path == "/api/chats/new":
        return _direct_json(environ, start_response, "200 OK", chats_api.new_chat(uid, ai_id))

    if method == "POST" and path == "/api/chats/rename":
        conversation_id = str(data.get("conversation_id", "")).strip()
        if not conversation_id:
            return _direct_json(environ, start_response, "400 Bad Request", {"ok": False, "error": "conversation_id required"})
        if not chats_api.rename_chat(uid, ai_id, conversation_id, data.get("title")):
            return _direct_json(environ, start_response, "400 Bad Request", {"ok": False, "error": "Unable to rename conversation"})
    ...
    if method == "POST" and path == "/api/chats/open":
        ...
        if result is None:
            return _direct_json(environ, start_response, "404 Not Found", {"ok": False, "error": "Conversation not found"})
...
def application(environ, start_response):
    ...
    status = f"{parts[1]} {parts[2]}" if len(parts) >= 3 else "500 Internal Server Error"
    response_headers = []
    for line in lines[1:]:
        if ":" not in line:
            continue
