...
async function loadMemory(){try{const j=await apiGet('/api/user');$('chat').innerHTML='';(j.conversation||[]).forEach(c=>{if(c.user)add(c.user,'user',c.timestamp,c.user);if(c.image)addImage(c.image,c.user?'user':'AI',c.timestamp);if(c.AI)add(c.AI,'AI',c.timestamp,c.user||'')})}catch(e){console.log(e)}}
