...
function addMessageActions(container,text,message,type){
 ...
 const sendFeedback=async(rating,button)=>{
   ...
   }catch(e){console.error('Feedback failed',e);button.classList.remove('selected')}
 };
...
function add(text,type,time=new Date(),message=''){
 ...
 d.appendChild(n);const t=document.createElement('span');t.className='timestamp';t.innerText=formatTime(time);d.appendChild(t);addMessageActions(d,text,message,type);$('chat').appendChild(d);$('chat').scrollTop=$('chat').scrollHeight;
}
...
// Conversation records historically used both `ai` and `AI`, and some older
// records used `time` while the UI expected `timestamp`. Accept all variants
// so moving storage does not make the assistant side disappear from the UI.
async function loadMemory(){try{const j=await apiGet('/api/user');$('chat').innerHTML='';(j.conversation||[]).forEach(c=>{const user=c.user??(c.role==='user'?c.content:'');const ai=c.ai??c.AI??(c.role==='assistant'?c.content:'');const stamp=c.time??c.timestamp??new Date();if(user)add(user,'user',stamp,user);if(c.image)addImage(c.image,user?'user':'AI',stamp);if(ai)add(ai,'AI',stamp,user||'')})}catch(e){console.log(e)}}
async function send(){const text=$('msg').value.trim(),file=$('chatImage').files[0];if(!text&&!file)return;lastActivity=Date.now();if(text)add(text,'user',new Date(),text);if(file)addImage(URL.createObjectURL(file),'user');$('msg').value='';$('chatImage').value='';$('send').disabled=true;updateTypingIndicator($('aiName').innerText||'AI');$('typing').style.display='block';try{let r;if(file){const f=new FormData();f.append('message',text);f.append('image',file);r=await fetch('/chat',{method:'POST',body:f})}else r=await fetch('/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text})});const j=await r.json();if(!r.ok)throw Error(j.error||'Chat failed');add(j.reply||'AI did not reply','AI',new Date(),text);if(j.image)addImage(j.image,'AI')}catch(e){add('AI error: '+e.message,'AI',new Date(),text)}finally{$('send').disabled=false;$('typing').style.display='none';$('msg').focus()}}
