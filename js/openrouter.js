(function(){
 ...
 window.updateTokenProviderUI=function(){
  ...
  if(p==='openrouter'){
   ...
   const token=document.getElementById('hfTokenInput');if(token)token.placeholder='Enter this AI\'s OpenRouter API key';
   ...
   const current=document.getElementById('apiModelInput')?.value||'openrouter/free';
   setModelControl(freeModels,current);loadFreeModels(current);
   return;
  }
 ...
 window.fillSettings=function(s){
  ...
  if(data.api_provider==='openrouter'){
   data.openai_token=data.openrouter_token||data.api_token||data.openai_token||'';
   data.api_provider='openrouter';
   data.api_endpoint=data.api_endpoint||OPENROUTER_ENDPOINT;
   data.api_model=data.api_model||'openrouter/free';
  }
  if(typeof baseFill==='function')baseFill(data);
  if((s||{}).api_provider==='openrouter'){
   const sel=document.getElementById('aiTokenProviderInput');if(sel)sel.value='openrouter';
   const token=document.getElementById('hfTokenInput');if(token)token.value=s.openrouter_token||s.api_token||s.openai_token||'';
   window.updateTokenProviderUI();
  }
 }
 window.addEventListener('load',function(){if(provider()==='openrouter')window.updateTokenProviderUI()});
})();
