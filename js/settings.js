function updateTokenProviderUI(){
 ...
 if(local){if(endpoint)endpoint.value='';if(model)model.value='SmolLM2-1.7B-Instruct-Q3_K_M'}
 ...
 if(model)model.placeholder=google?'Leave blank to auto-select an available Gemini model':openrouter?'Select a free OpenRouter model':local?'SmolLM2-1.7B-Instruct-Q3_K_M':'gpt-4o-mini';
 const compatibility=$('apiCompatibilityText');if(compatibility)compatibility.innerText=local?'This AI runs SmolLM2 locally on the AI-Server. No external API is used.':google?'This AI uses its own Google AI Studio API key.':openrouter?'This AI uses its own OpenRouter API key and OpenRouter free-model routing.':openai?'This AI uses its own OpenAI API token.':'This AI uses its own Hugging Face API token.';
...
async function saveSettings(){
 ...
 try{
  ...
  if(provider==='local'){s.hf_token='';s.openai_token='';s.google_token='';s.openrouter_token='';s.api_endpoint='';s.api_model='SmolLM2-1.7B-Instruct-Q3_K_M'}
