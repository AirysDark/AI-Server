...
from core.config import AUTH_FILE, SESSIONS_FILE, STORAGE_DIR, PUBLIC_URL
from core.storage import load_json, save_json

RESET_FILE = os.path.join(STORAGE_DIR, "password_resets.json")
RESET_TTL = 30 * 60
...
def request_reset(email):
    ...
    user = next(((uid, record) for uid, record in accounts.get("users", {}).items() if normalize_email(record.get("email")) == email), None)
    if not user:
        return True
    uid = user[0]
